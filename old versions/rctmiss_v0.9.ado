...
syntax, [ ///
    ...
    debug mmstore(passthru) SANDwich keepmat(passthru) dicmd /// undocumented options
    ]
...
if `neffvalue'==0 {
	...
	mata {
		residS = st_data(., "`residS'")
		xS = st_data(., "`xSlist' `one'")
		residP = st_data(., "`residP'")
		xP = st_data(., "`xPlist' `one'")

		Binv=st_matrix("`Binv'")
		v=st_matrix("`v'")
		BSS=st_matrix("`BSS'")

		U = (diag(residS)*xS, diag(residP)*xP)
		A = (I(`pS'), J(`pS',`pP',0))
		ABinvU = A*Binv*U'
		vinv = invsym(v)
		inf = diagonal(ABinvU'*vinv*ABinvU)
		st_store(., "`influence'obs", inf)

		BSSinv=luinv(BSS)
		inf2 = diagonal(xS*BSSinv'*vinv*BSSinv*xS')
		st_store(.,"`influence'full",inf2)
	}
