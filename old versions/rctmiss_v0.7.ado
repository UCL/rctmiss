*! version 0.7   Ian White    15may2012 
/*******************************************************************************
HISTORY
*! version 0.7   15may2012
    Trying to get CIs right too, by posting dof
...
mat l np, title("The NP matrix")
    ...
    ereturn scalar neff = `neff'
    ereturn scalar pstar = `pstar'

    noi di "ereturn post `b' `V', depname(`yvar') obs(`n') esample(`touse') `dof'"
    ereturn post `b' `V', depname(`yvar') obs(`n') esample(`touse') `dof'
    ...
    * sort out y-variable
    gen zcrit = invnorm(.5+`level'/200) if `dof' == .
    replace zcrit = invttail(dof,.5-`level'/200) if `dof' != .
    if "`eform'"!="" {
        gen exp_b = exp(b)
...
// CORRECTION MODEL 
// fitted to all obs, but need to pretend fitted only to those with obs y
tempvar mdz
if "`delta'"!="" {
    ...
    mat `b' = `b' + e(b)
    mat `v' = `v' + e(V)*`factorI'/`factorA'  // why???
}
...
*di "in pmm_reg"
*eret list
mat np = (e(N),e(N)-e(df_r)) // TO BE IMPROVED
...
mat np = (`neff',`dfcorrection') // TO BE IMPROVED
