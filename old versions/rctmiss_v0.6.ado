*! version 0.6   Ian White    28aug2011
/*******************************************************************************
HISTORY
version 0.6   28aug2011
...
syntax, [ ///
    ...
    senstype(string) list LISTOPTions(string) clear savewt(string) eform(string) saving(string) replace /// output and display options
...
if "`smdelta'"!="" {
    * SELECTION MODEL / IPW METHOD
    local maincmd sm_ipw `command' b(`b') v(`V') `savewtopt' `sw'
    local deltaname1 SM 
}
if "`pmmdelta'"!="" {
...
if "`sens'"=="" {
    ...
    * SENSITIVITY ANALYSIS
    cap numlist "`delta'"
    if _rc {
        di as error "Syntax with sens(): pmmdelta([log] numlist) or smdelta([log] numlist) "
        exit 198
    }
    ...
    foreach del of numlist `delta' {
        ...
        foreach type in `typelist' {
            ...
            if `type'==1 local deltavar cond(`sens'==`randint',`logdel',0)
            if `type'==2 local deltavar `logdel'
            if `type'==3 local deltavar cond(`sens'==`randcon',`logdel',0)
    ...
    * sort out y-variable
    if "`eform'"!="" {
        gen exp_b = exp(b)
        gen exp_b_low = exp(b-1.96*se)
        gen exp_b_upp = exp(b+1.96*se)
...
// PARSE
syntax anything [if] [in], delta(string) [debug noCONStant b(string) V(string)]
...
`qui' `cmd2' `y' `xlist' if `touse', `opts' `constant' robust
...
`qui' `cmd2' `ystar' `xlist' if `touse', `opts' `constant' robust
...
prog def sm_ipw
version 10
syntax anything [if] [in], [delta(string) robust debug noSUMwt savewt(string) noCONStant b(string) V(string) nosw]
